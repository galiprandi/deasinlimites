...
## 2025-05-16 - [Enhanced Search & Share Interactions]
**Learning:** Search inputs should support the 'Escape' key to clear the query and blur the input field, providing a standard and efficient way to reset the search state. Additionally, social sharing links benefit from descriptive `title` attributes to provide native tooltips for desktop users, complementing `aria-label` for screen readers.
**Action:** Implement 'Escape' key support in search inputs and ensure all icon-only social links have both `aria-label` and `title` attributes for universal discoverability.
