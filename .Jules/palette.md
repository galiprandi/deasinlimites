...
## 2025-05-15 - [Visual Scroll Progress]
**Learning:** For long-form content, a reading progress bar provides immediate feedback on remaining content, reducing abandonment. Throttling scroll events with `requestAnimationFrame` ensures smooth performance without layout thrashing.
**Action:** Use a fixed progress bar with `var(--color-secondary)` on article detail pages to improve content consumption feedback.
