...
- **Identidad de Marca Dinámica**: Utilizar el componente `TitleAccent` para aplicar acentos cromáticos programáticos en títulos de sección, envolviendo automáticamente la última palabra en un `<span>`.
- **Métricas de Lectura**: Implementar la visualización del tiempo estimado de lectura en todas las tarjetas de artículos y cabeceras de detalle, utilizando el icono `clock.svg` y la utilidad `getReadingTime`.
...
- **Dynamic Title Accents**: Utilizar el componente `TitleAccent` para asegurar que el acento cromático se aplique consistentemente a la última palabra de los títulos principales y de sección.
...
- **Reading Time Metrics**: Incluir métricas de tiempo de lectura en `PostCard` y cabeceras de artículos utilizando la utilidad `getReadingTime` (basada en 200 ppm) y el icono `clock.svg`.
