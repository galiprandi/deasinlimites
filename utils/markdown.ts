...
/**
 * Valida que una ruta no contenga secuencias de escape de directorio (path traversal).
 */
function validatePath(pathPart: string): void {
  if (pathPart.includes("..") || pathPart.includes("\\")) {
    throw new Error(`Invalid path segment: ${pathPart}`);
  }
}
